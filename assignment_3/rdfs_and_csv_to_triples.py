...
def get_subset_df(entity_name: str):
    return pd.concat(
        (
            ...
            subset_column_match(
                objects_df, r"rdfs:domain{URIRef}[0]", r"rdfs:domain{URIRef}[1]", f":{entity_name}"
            ),
        )
    )
...
full_graph = Graph()
def df_to_rdf(csv_data: pd.DataFrame, rdf_schema: pd.DataFrame, key_column: str, entity_name: str, is_bnode: bool):
    for _, row in csv_data.iterrows():
        ...
        for index, rdf_row in rdf_schema.iterrows():
            if rdf_row[r"rdf:type{URIRef}"] in ("owl:DatatypeProperty", "owl:ObjectProperty"):
            # if rdf_row[r"rdf:type{URIRef}"] == "owl:DatatypeProperty":
                index = index.replace(":", "")
                if index == key_column or (is_bnode and rdf_row[r"rdf:type{URIRef}"] == "owl:ObjectProperty"):
                    continue
                if index in csv_data.columns:
                    full_graph.add(
                        (
                            ...
                            Literal(
                                convert_to_correct_type(row[index], rdf_row[r"rdfs:range{URIRef}"]) 
                                if rdf_row[r"rdf:type{URIRef}"] == "owl:DatatypeProperty"
                                else row[index]
                            ),
...
def add_csv_to_graph(filepath: str, column_rename: dict[str, str], key: str, name: str, entity_name: str = None, is_bnode: bool = False):
    ...
    if key not in data_df.columns:
        if key == "programmeInstanceID":
            data_df["programmeInstanceID"] = data_df["programmeInstanceProgrammeCode"] + "-" + data_df["programmeInstanceStudyYear"]
...
add_csv_to_graph("data/Assigned_Hours.csv", assigned_hours_rename, "teachesTeacherID", "Teaches", is_bnode=True)
...
add_csv_to_graph("data/Course_Instances.csv", course_instance_rename, "courseOfferingInstanceID", "CourseOffering")
...
add_csv_to_graph("data/Course_plannings.csv", course_plannings_rename, "courseOfferingInstanceID", "CourseOffering")
...
add_csv_to_graph("data/Courses.csv", courses_given_by_rename, "courseCode", "Division", "GivenBy", is_bnode=True)
...
add_csv_to_graph("data/Programme_Courses.csv", programme_courses_rename, "programmeInstanceProgrammeCode", "ProgrammeInstance")
programme_courses_rename = {
    "Programme code": "programmeInstanceProgrammeCode",
    # "Study Year": "programmeInstanceStudyYear",
    # "Academic Year": "programmeInstanceAcademicYear",
...
add_csv_to_graph("data/Programme_Courses.csv", programme_courses_rename, "programmeInstanceProgrammeCode", "ProgrammeCourses", is_bnode=True)
...
add_csv_to_graph("data/Registrations.csv", registrations_rename, "registrationsStudentID", "Registrations", is_bnode=True)
...
add_csv_to_graph("data/Reported_Hours.csv", reported_hours_rename, "teachesTeacherID", "Teaches", is_bnode=True)
...
add_csv_to_graph("data/Senior_Teachers.csv", senior_teachers_rename, "teacherID", "Teacher", "SeniorTeacher")

teaching_assistants_rename = senior_teachers_rename
add_csv_to_graph("data/Teaching_Assistants.csv", teaching_assistants_rename, "teacherID", "Teacher", "TeachingAssistant")
